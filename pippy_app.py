...
class PippyActivity(ViewSourceActivity):
    ...
    # Create a new window
        ...
        if self.ai_window is not None:
            ...
            self.ai_window.set_default_size(750, 815)
            self.ai_window.set_resizable(False)
            self.ai_window.set_decorated(False)
            self.ai_window.set_transient_for(self.get_toplevel())
            self.ai_window.move(1300, 200)  # The first argument is the x-coordinate (left-right), the second is the y-coordinate (up-down)
        ...
        # Show the window and all its contents
            ...
            # Create a label for the user message
                ...
                user_message.set_max_width_chars(25)
                user_message.get_style_context().add_class("user-message")
                user_message.set_xalign(0)  # Align left

            # Add the user message to the message box
                message_box.pack_start(user_message, False, False, 0)
                message_box.show_all()

            # Send the message (placeholder for actual sending logic)
                self.send(message, message_box)

        # Connect the send button to a function to handle the message
    ...
    def _simulate_ai_response(self, message_box):
        ...
        ans="Chatbot: This is a response."
        ...
        bot_response.set_xalign(0)  # Align left

    # Add the bot response to the message box
